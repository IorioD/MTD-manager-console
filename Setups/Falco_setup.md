...
## 1. Install Falco (probes installed on each node)
   ...
   ```
   Under the directory `/etc/falco/` you'll find `config.d` folder containing the configuration file of the engine (ebpf, kernel, or legacy), `falco.yaml` containing the full falco configuration file, and `falco_rule.yaml` containing the default falco rules. 
